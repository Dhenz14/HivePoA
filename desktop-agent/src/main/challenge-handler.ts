...
import { computeProofHash, getBlockCids } from './poa-crypto';
...
export class ChallengeHandler {
  ...
  constructor(kuboApiUrl: string, pubsub: PubSubBridge, myUsername: string, config: ConfigStore) {
  ...
  async handleChallenge(challenge: ChallengeMessage): Promise<void> {
    ...
    try {
      ...
      await this.pubsub.publish(CHALLENGE_TOPIC, response);
  ...
  private cleanup(): void {
    const cutoff = Date.now() - 120000; // 2 minute retention
